/**
 * Cuelume 0.2.2 — MIT
 * https://github.com/bytonuzzz/cuelume
 * Vendored as a single IIFE for the OGFonts Inspector content script.
 * No audio files. Sounds are synthesized with the Web Audio API.
 */
(function (root) {
  if (root.Cuelume) return;
  ...
  function play(sound, options) {
    ...
    const playVolume = globalVolume * normalizeVolume(options && options.volume, 1);
  ...
  root.Cuelume = { play, setEnabled, setVolume, sounds: Object.keys(RECIPES) };
